...
{
    return (semaphore_context_t*)(kernal_member_unified_id_toContainerAddress(id));
}
...
{
    return (list_t*)kernal_member_list_get(KERNAL_MEMBER_SEMAPHORE, KERNAL_MEMBER_LIST_SEMAPHORE_LOCK);
}
...
{
    return (list_t*)kernal_member_list_get(KERNAL_MEMBER_SEMAPHORE, KERNAL_MEMBER_LIST_SEMAPHORE_UNLOCK);
}
...
{
    return kernal_member_unified_id_isInvalid(KERNAL_MEMBER_SEMAPHORE, id);
}
...
static void _semaphore_callback_fromTimeOut(os_id_t id)
{
    ...
    kernal_thread_entry_trigger(kernal_member_unified_id_timerToThread(id), id, PC_SC_TIMEOUT, _semaphore_schedule);
}
...
{
    return (u32_t)(_semaphore_id_isInvalid(id) ? (0u) : (id - kernal_member_id_toUnifiedIdStart(KERNAL_MEMBER_SEMAPHORE)) / sizeof(semaphore_context_t));
}
...
os_id_t _impl_semaphore_init(u8_t availableCount, u8_t limitationCount, const char_t *pName)
{
    ...
    return kernal_privilege_invoke(_semaphore_init_privilege_routine, arguments);
}
...
u32p_t _impl_semaphore_take(os_id_t id, u32_t timeout_ms)
{
    ...
    u32p_t postcode = kernal_privilege_invoke(_semaphore_take_privilege_routine, arguments);
...
u32_t _impl_semaphore_give(os_id_t id)
{
    ...
    return kernal_privilege_invoke(_semaphore_give_privilege_routine, arguments);
}
...
u32_t _impl_semaphore_flush(os_id_t id)
{
    ...
    return kernal_privilege_invoke(_semaphore_flush_privilege_routine, arguments);
}
...
static u32_t _semaphore_init_privilege_routine(arguments_t *pArgs)
{
    ...
    semaphore_context_t *pCurSemaphore = (semaphore_context_t *)kernal_member_id_toContainerStartAddress(KERNAL_MEMBER_SEMAPHORE);
    os_id_t id = kernal_member_id_toUnifiedIdStart(KERNAL_MEMBER_SEMAPHORE);
    ...
    do {
        ...
        pCurSemaphore++;
        id = kernal_member_containerAddress_toUnifiedid((u32_t)pCurSemaphore);
    } while ((u32_t)pCurSemaphore < (u32_t)kernal_member_id_toContainerEndAddress(KERNAL_MEMBER_SEMAPHORE));
...
static u32_t _semaphore_take_privilege_routine(arguments_t *pArgs)
{
    ...
    {
        /* No availabe count */
        postcode = kernal_thread_exit_trigger(pCurThread->head.id, id, _semaphore_list_blockingHeadGet(id), timeout_ms, _semaphore_callback_fromTimeOut);
...
static u32_t _semaphore_give_privilege_routine(arguments_t *pArgs)
{
    ...
    {
        postcode = kernal_thread_entry_trigger(pSemaphoreHighestBlockingThread->head.id, id, PC_SC_SUCCESS, _semaphore_schedule);
    }
...
static u32_t _semaphore_flush_privilege_routine(arguments_t *pArgs)
{
    ...
    {
        pCurSemaphore->availableCount++;
        postcode = kernal_thread_entry_trigger(pCurThread->head.id, id, PC_SC_SUCCESS, _semaphore_schedule);
        if (PC_IER(postcode))
        {
            break;
        }
...
static void _semaphore_schedule(os_id_t id)
{
    ...
    if (kernal_member_unified_id_toId(pEntryThread->schedule.hold) == KERNAL_MEMBER_SEMAPHORE)
    {
        ...
        if ((pEntry->result == PC_SC_SUCCESS) || (pEntry->result == PC_SC_TIMEOUT))
        {
            ...
            else if (kernal_member_unified_id_toId(pEntry->release) == KERNAL_MEMBER_SEMAPHORE)
            {
